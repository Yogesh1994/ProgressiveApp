...
@IonicPage()
@Component({
  selector: 'page-invoice-report',
  templateUrl: 'invoice-report.html',
})
export class InvoiceReportPage {
  ...
  onSubmit() {
    ...
    this.storage.get('userData').then((val) => {
      ...
      let link = "http://www.progressiveit.in/progressive_app/invoiceReportMail.php?period=" + periodType + "&email=" + userEmail + "";
      this.http.get(link)
        .map(res => res.json()).subscribe((data) => {
          ...
          if (data.server_response == 'true') {
            let toast = this.toastCtrl.create({
              message: 'Invoice Report Sent On Mail',
              duration: 2000,
              position: 'bottom'
            });
            toast.present(toast);
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad InvoiceReportPage');
  }
